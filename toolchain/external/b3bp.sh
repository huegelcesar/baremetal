...
function __b3bp_log () {
  ...
  while IFS=$'\n' read -r log_line; do
    echo "${echo_opt[@]}" -e "$(date -u +"%Y-%m-%d %H:%M:%S UTC") ${color}$(printf "[%9s]" "${log_level}")${color_reset} ${log_line}" 1>&2
  done <<< "${@:-}"
}
...
while read -r __b3bp_tmp_line; do
  if [[ "${__b3bp_tmp_line}" =~ ^- ]]; then
    ...
    if [[ "${__b3bp_tmp_line}" =~ ^-- ]]; then
      __b3bp_tmp_opt=${__b3bp_tmp_long_opt}
    fi
    ...
    if [[ "${__b3bp_tmp_line}" =~ \[.*\] ]]; then
      ...
      printf -v "__b3bp_tmp_has_arg_${__b3bp_tmp_opt%%:*}" '%s' "1"
    elif [[ "${__b3bp_tmp_line}" =~ \{.*\} ]]; then
      __b3bp_tmp_opt="${__b3bp_tmp_opt}:" # add : if opt has arg
      __b3bp_tmp_init=""  # it has an arg. init with ""
      # remember that this option requires an argument
      printf -v "__b3bp_tmp_has_arg_${__b3bp_tmp_opt%%:*}" '%s' "2"
    else
      __b3bp_tmp_init="0" # it's a flag. init with 0
      printf -v "__b3bp_tmp_has_arg_${__b3bp_tmp_opt%%:*}" '%s' "0"
    fi
  ...
  printf -v "arg_${__b3bp_tmp_opt%%:*}" '%s' "${__b3bp_tmp_init}"
done <<< "${__usage:-}"
...
if [[ "${__b3bp_tmp_opts:-}" ]]; then
  ...
  while getopts "${__b3bp_tmp_opts}" __b3bp_tmp_opt; do
    ...
    __b3bp_tmp_varname="arg_${__b3bp_tmp_opt%%:*}"
